...
start /wait "" MSTeamsSetup.exe /provisionsilent

echo [3/6] Instalando ONLYOFFICE (Silencioso)...
start /wait "" ONLYOFFICEDesktopInstaller.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART

echo [4/6] Abrindo instalador do GLPI Agent (Manual)...
start /wait "" msiexec /i GLPI-Agent-1.7.1-x64.msi

echo [5/6] Baixando e instalando LibreOffice (Silencioso)...
echo Baixando instalador oficial mais recente do LibreOffice...
powershell -Command "Invoke-WebRequest -Uri 'https://www.libreoffice.org/donate/dl/win-x86_64/7.6.4/en-US/LibreOffice_7.6.4_Win_x64.msi' -OutFile 'LibreOfficeSetup.msi'"

echo Instalando LibreOffice...
start /wait "" msiexec /i LibreOfficeSetup.msi /qn /norestart

echo [6/6] Abrindo AnyDesk (Manual)...
start "" AnyDesk.exe
